...
class TwitterSearch:
    ...
    write_f = codecs.open("1.csv", 'w','utf_8_sig')
    write_f.write('date, user, name, text, sentiment, revise_score')
    write_f.close
    ...
    def search(self, query, max_position=None):
        ...
        write_f = codecs.open("1.csv", 'a','utf_8_sig')
        # open the file with a for appending
        write_f.write(codecs.BOM_UTF8)
    ...
    def parse_tweets(items_html):
        ...
        for li in soup.find_all("li", class_='js-stream-item'):
            ...
            if text_p is not None:
                ...
                tweet['sentiment']=afinn.score(tweet['text'])
                if tweet['sentiment']>=0:
                        tweet['revised_sentiment']=1
                else:
                        tweet['revised_sentiment']=-1
...
class TwitterSearchImpl(TwitterSearch):
    ...
    def save_tweets(self, tweets, write_f):
        ...
        for tweet in tweets:
                write_f.write(('\n"%s","%s","%s","","%.1f","%d"' % (
                str(datetime.date(dt_obj_start)), tweet['user_screen_name'], 
                tweet['text'].replace('"', '').replace('\n', ' ').replace('\r', ' ').replace('  ', ' ').replace(',','').replace('…',''), tweet['sentiment'],tweet['revised_sentiment']
            )))
...
query_here_1=raw_input("please input key words: ")
...
while dt_obj_start <= dt_obj_stop: # As long as the stop date has not been reached
        ...
        if __name__ == '__main__':
            ...
            twit.search(str(query_here_1) + " since:" + str(datetime.date(dt_obj_start)) + " until:" + str(datetime.date(dt_obj_slice)),
                    max_position=None)
